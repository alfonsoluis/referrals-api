  ...
  describe('API authentication', () => {
    ...
    test('API should be accessible with JWT', async () => {
      ...
      const results = await Promise.all([
        request(app).get('/api/user').set('Authorization', jwt),
        request(app).get(`/api/user/${id}`).set('Authorization', jwt),
        request(app).post('/api/user').set('Authorization', jwt),
        request(app).put(`/api/user/${id}`).set('Authorization', jwt),
        request(app).delete(`/api/user/${id}`).set('Authorization', jwt),
      ])
